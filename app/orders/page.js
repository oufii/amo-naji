...
export default function OrdersPage() {
  ...
  async function fetchOrders() {
    ...
    if (data) {
      // فلترة محلية تمنع اختفاء الطلبات إذا كان عمود status غير موجود بالأساس
  ...
  const handleCompleteOrder = async (id) => {
    ...
    if (error) {
      // إذا لم يكن عمود status موجوداً، نقوم بعمله محلياً أو إظهار تنبيه
      alert('تم تحديث الطلب محلياً (تأكد من إضافة عمود status في سوبابيس إن أردت حفظه دائمياً)');
      fetchOrders();
    } else {
      fetchOrders();
    }
